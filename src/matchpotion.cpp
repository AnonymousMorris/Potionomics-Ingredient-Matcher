...
int calc_index(std::array<int, 5> ingredient) {
    int val = 0;
...
void calc_dp() {
    ...
    dp.assign(calc_index(mx_mags) + 1, 0);
    dp[0] = 1;
    ...
    for (std::array<int, 5> ingredient : ingredients) {
        ...
        int offset = calc_index(ingredient);
    ...
    for (int i = sz(dp) - 1; i > 0; i -= offset) {
        for (int ii = mx_ingredients; ii > 0; ii--) {
            ...
            for (int iii = 0; iii < sz(ingredients); iii++) {
                ...
                int crn_val = calc_index(ingredients[iii]);
                while ((remaining > 0) && (crn_idx - crn_val >= 0) &&
                       (((dp[crn_idx - crn_val] >> (remaining - 1)) & 1) == 1)) {
                    bool valid = 1;
...
int main(int argv, char* args[]) {
    ...
    if (argv < 3) {
        ...
        cauldron_mag_limit = stoi(args[2]);
    }
    ...
    if (argv < 4) {
        ...
        cauldron_ingredient_limit = stoi(args[3]);
    }
